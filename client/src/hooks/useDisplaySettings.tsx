...
// Define translation type with all available options from database
export type TranslationType = 'tajik' | 'tj_2' | 'tj_3' | 'farsi' | 'russian'; 
...
const DisplayContext = createContext<DisplayContextType>({
  wordByWordMode: true, // Always enabled by default
...
export function DisplayProvider({ children }: DisplayProviderProps) {
  ...
  const [wordByWordMode, setWordByWordMode] = useState(() => 
    loadFromLocalStorage('wordByWordMode', true)); // Always true by default
