...
export interface IStorage {
  ...
  getWordByWordForVerse(verseKey: string): Promise<any[]>;
}
...
export class MemStorage implements IStorage {
  ...
  private wordByWordData: Map<string, any[]>;  // Store word-by-word data by verse key
...
export class PostgresStorage implements IStorage {
  ...
  async getVersesBySurah(surahId: number): Promise<Verse[]> {
    return await db.select({
      ...
      page: verses.page,
      juz: verses.juz,
      unique_key: verses.unique_key,
      transliteration: verses.transliteration,
      tafsir: verses.tafsir,
      tj_2: verses.tj_2,
      tj_3: verses.tj_3,
      farsi: verses.farsi,
      russian: verses.russian
    })
    .from(verses)
    .where(eq(verses.surah_id, surahId))
  ...
  async getVerseByKey(key: string): Promise<Verse | undefined> {
    ...
    .from(verses)
    .where(eq(verses.unique_key, key))
    .limit(1);
      ...
      if (functionExists) {
        ...
        for (const row of advancedResults) {
          const verse: Verse = {
            ...
            transliteration: (row as any).transliteration,
            tafsir: (row as any).tafsir,
            tj_2: (row as any).tj_2,
            tj_3: (row as any).tj_3,
            farsi: (row as any).farsi,
            russian: (row as any).russian
          };
          results.push(verse);
        }
